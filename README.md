...
> **Agentic expansion (this fork).** Beyond the stock speedrun, this fork adds two
> tracks toward agentic, GLM-class capability — **Track A** (real coding capability by
> post-training Qwen3-8B with verl, in `posttrain/`) and **Track B** (the same agentic
> tool-use + coding RL built *from scratch* on nanochat: `nanochat/rl_core.py`,
> `nanochat/tools.py`, `scripts/agent_rl.py`, `tasks/coding_env.py`). Start here:
> **[AGENTIC.md](AGENTIC.md)**. Local tests (no GPU): `bash runs/test_agentic.sh`.
