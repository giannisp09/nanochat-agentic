...
from tasks.tooltrace import CodingToolTrace # agentic-coding tool-use SFT seed (Track B)
...
parser.add_argument("--coding-epochs", type=int, default=0, help="epochs of coding tool-use trajectories (CodingToolTrace) to seed agentic-coding RL (0=off)")
...
train_tasks = [
    ...
    *[CodingToolTrace(split="train") for _ in range(args.coding_epochs)], # tool-use coding traces (opt-in; bundle is small, oversample or point at a bigger corpus)
    SimpleSpelling(size=200000, split="train"), # 200K rows of Simple Spelling (e.g. spell the word 'apple')
    SpellingBee(size=80000, split="train"), # 80K rows of Spelling Bee (e.g. how many 'r' are in 'strawberry'?)
]
train_dataset = TaskMixture(train_tasks)
print0(f"Training mixture: {len(train_dataset):,} rows (MMLU x{args.mmlu_epochs}, GSM8K x{args.gsm8k_epochs}, Coding x{args.coding_epochs})")
